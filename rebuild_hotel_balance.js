...
async function main() {
    ...
    // Content ends right before CTA
    const ctaIdx = old.indexOf('<!-- CTA Box -->');
    const endIdx = ctaIdx > -1 ? ctaIdx : old.indexOf('<!-- FOOTER');

    let hotelContent = old.substring(firstPIdx, endIdx).trim();

    // EXACTLY BALANCE DIVS
    let opens = (hotelContent.match(/<div(\s|>)/g) || []).length;
    let closes = (hotelContent.match(/<\/div>/g) || []).length;
    console.log('Original extracted DIV layout -> opened:', opens, 'closed:', closes);

    // If there are more closes than opens, remove trailing closures.
    while (closes > opens && hotelContent.endsWith('</div>')) {
        hotelContent = hotelContent.substring(0, hotelContent.lastIndexOf('</div>')).trim();
        closes--;
    }
    // If still more closes than opens (meaning they aren't trailing but inside), we just hack strip from end
    while (closes > opens) {
        const lastIdx = hotelContent.lastIndexOf('</div>');
        if (lastIdx > -1) {
            hotelContent = hotelContent.substring(0, lastIdx) + hotelContent.substring(lastIdx + 6);
            closes--;
        } else {
            break; // Shouldn't happen
        }
    }
    // If there are less closes than opens, append `</div>`
    while (closes < opens) {
        hotelContent += '\n</div>';
        closes++;
    }

    console.log('Balanced DIV layout -> opened:', opens, 'closed:', closes);
    ...
    // Let's verify final newHtml div count:
    const finalOpens = (newHtml.match(/<div(\s|>)/g) || []).length;
    const finalCloses = (newHtml.match(/<\/div>/g) || []).length;
    console.log('Final New HTML DIV count -> opened:', finalOpens, 'closed:', finalCloses);
